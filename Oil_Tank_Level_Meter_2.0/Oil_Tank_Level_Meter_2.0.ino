/*
  Stepper Motor SMCC-547 for Oil Tank Level Indicator v. 2.0
...
int outputPin[5] = { 0, 5, 6, 7, 8 };  // stepper motor pins
int level;  // the current level of the oil tank
int oldLevel = 0; // the previous reading of the level
int motordelay = 10; //standard stepper motor delay
byte oilLevel;
int delaytime = 100;
int hdr;  // the input payload from the sensor via the RF12
boolean motInitPin = 1;
...
void setup () {
  ...
  pinMode(5, OUTPUT); // to A input on motor
  pinMode(6, OUTPUT); // to B input on motor
  pinMode(7, OUTPUT); // to C input on motor
  pinMode(8, OUTPUT); // to D input on motor
  ...
  pinMode(3, INPUT); // position sensor
  ...
  while (motInitPin == HIGH) {
    rotateLeft();
    motInitPin = digitalRead(3);
  }
...
void loop () {
  ...
  if (hdr == 6) {
    ...
    if (oldLevel == oilLevel) // if there is no change, do nothing
    {
      delay(motordelay);
    }
    else if (oldLevel > oilLevel) // if level goes down, rotate left
    {
      rotateLeft();
